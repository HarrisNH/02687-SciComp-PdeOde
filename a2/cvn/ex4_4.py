    ...
    while t < T:
        ...
        for i in range(1, N):
            ...
            if i >= 2 and i <= N-2:
                ...
                if ui >= 0:
                    adv = ui * (3*ui - 4*ul + ull) / (2*h)
                else:
                    adv = ui * (-urr + 4*ur - 3*ui) / (2*h)

                # fourth-order central diffusion
                diff = eps * (-urr + 16*ur - 30*ui + 16*ul - ull) / (12*h**2)
...
def main():
    ...
    fig.suptitle(
        r"Burgers — higher-order stencil on uniform grid "
        r"$\epsilon=0.01/\pi$",
        fontsize=12, fontweight="bold")
    fig.tight_layout()
    fig.savefig("burgers_part4.png", dpi=130)
    print("\nPlot saved to burgers_part4.png")
    plt.show()
