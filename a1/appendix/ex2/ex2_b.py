...
def assemble_F_J(u, x, eps, s_func=None, bc_left=None, bc_right=None):
    ...
    if s_func is None:
        ...
        s = s_func(x, eps)
    if bc_left is None:
        bc_left = u[0]  # fallback (not ideal, but safe)
    if bc_right is None:
        bc_right = u[-1]
    ...
    for i in range(1, N - 1):
        ...
        F[i] = eps * uxx + u0i * (ux - 1.0) - s[i]  # why -s[i]
...
def convergence_plotter(
    ...
    plt.loglog(h_coarse, ref2, "--", label=r"reference $\propto h^2$")
...
convergence_plotter(
    m, levels, t_start, t_end, solve_actual, plt_show=True, plt_title="ex2b_f0"
)
