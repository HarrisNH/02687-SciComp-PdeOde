...
def main():
    m = 2**6 - 1
    U = np.zeros(m * m)
    F = fc_a.construct_b(m, fc_a.f_rhs, fc_a.u_exact)   # TODO: Form the right-hand side
    epsilon = 1.0e-10
...
def Vcycle(U, omega, nsmooth, m, F):
    ...
    if m == 1:
        ...
        mc = (m - 1) // 2
        Ecoarse = Vcycle(np.zeros(mc * mc), omega, nsmooth, mc, Rcoarse) #what is Rcoarse. Is that F?. Why is it just zeros as input.
